...
public class StochasticLocalSearch {
	...
	private CentralizedPlan selectInitialSolution(TaskSet tasks) {
		...
		TaskAction firstTaskaction_pickup = new TaskAction(firstTask,TaskAction.PICK_UP);
		TaskAction firstTaskaction_delivery = new TaskAction(firstTask,TaskAction.DELIVERY);

		vehicleToFirstTaskAction.put(largestVehicle, firstTaskaction_pickup);

		taskActionToTaskAction.put(firstTaskaction_pickup, firstTaskaction_delivery);
		...
		while (!tasksList.isEmpty()) {
			...
			taskActionToTaskAction.put(
					new TaskAction(task, TaskAction.PICK_UP), 
					new TaskAction(task, TaskAction.DELIVERY));

			if (!tasksList.isEmpty()) {
				Task nextTask = tasksList.getFirst();
				taskActionToTaskAction.put(
						new TaskAction(task,TaskAction.DELIVERY), 
						new TaskAction(nextTask,TaskAction.PICK_UP));
			} 
		}
		taskActionToTaskAction.put(
